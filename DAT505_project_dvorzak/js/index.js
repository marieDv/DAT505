...
var renderer, scene, camera, extrudePath,
    frequencyData, audio, analyser, geometry, material, points, pipeSpline, direction, composer, lyrics, tempWord,
    countWords = 0
;
...
function changeSong(direction) {
    ...
    direction === "left" ? TweenMax.to(camera.position, 2, {x: temp - 1}) : TweenMax.to(camera.position, 2, {x: temp + 260}); //animate camera position to the next model
    direction === "left" ? TweenMax.to(camera.rotation, 2, {y: tempRot + 0.001}) : TweenMax.to(camera.rotation, 2, {y: tempRot - 0.003});
    direction === "left" ? document.getElementById("left").style.visibility = "hidden" : document.getElementById("right").style.visibility = "hidden"; //hide navigation buttons
    ...
    direction === "left" ? document.getElementById("songTitle").innerHTML = "Ta Reine" : document.getElementById("songTitle").innerHTML = "La Loi de Murphy"; //change song title
    direction === "left" ? document.getElementById("songTitle").style.marginLeft = "-4px" : document.getElementById("songTitle").style.marginLeft = "-57px"; //style adjustment
    initializeAudio();
}
...
function init() {
    ...
    addShaderLights(); //add lights for volumetric light shader
    initializeAudio(); //initzialize all web-audio specific functionalities
    for (let i = 0; i < planeVertices / 2; i++) {//call loop depending on the frequency range to create corresponding number of models
...
function loadBasicSurface(group, shadowGroup) {
    ...
    createLines(); // create CatmullRomCurve3 for extrusion
    ...
    var plane = new THREE.Mesh(tubeGeometry, material); //basic mesh
    var copyMesh = new THREE.Mesh(geom, shadowMaterial); //shadow-sibling that is responsible for the animation of the cast shadows
...
function addComposer() {
    ...
    occlusionComposer = new THREE.EffectComposer(renderer, occlusionRenderTarget); // add an additional composer for the occlusion layer that handles the shadows
    occlusionComposer.addPass(new THREE.RenderPass(scene, camera));
    pass = new THREE.ShaderPass(THREE.VolumetericLightShader); //add volumetriclightshader for rays
    ...
    composer = new THREE.EffectComposer(renderer); // composer for the main layer
...
function createLyrics() {
    ...
    lyrics = templyrics.innerHTML.split(" ");//get the innerHTML of the element, and split it on every space to get single words
    console.log(lyrics);

    for (let i = 0; i < lyrics.length; i++) {
        let temp = document.createElement("span");
        if (!(lyrics[i] === null) || !(lyrics[i] === " ")) {
            temp.innerHTML = lyrics[i] + " " //apply them to an array
        }
        temp.className = "line";
        document.getElementById("animatedLyrics").appendChild(temp); //and add the single spans to another element
    }
...
function mapLyrics(fd, i, child) {
    ...
    word[countWords].style.top = (child.scale.x * 30 * Math.random()) + "px"; //map the lyrics recording to the current frequence
    ...
    setTimeout(function () {
        for (let j = 0; j < word.length; j++) {
            if (word[j].classList.contains("toRemove")) { //and remove them after a certain amount of time
...
function colorChange(frequencyData, i, group) {
    ...
    if (frequencyData[i] > 20 && frequencyData[i] < 100) { //middletones
        let updateColor = new THREE.Color("rgb(" + 25 + "%, " + 25 + "%, " + 25 + "%)");//30 80
        group.children[i].material.color = updateColor;
    }
    if (frequencyData[i] > 85 && frequencyData[i] < 90) { //female lead
        let updateColor = new THREE.Color("rgb(" + 80 + "%, " + 80 + "%, " + 80 + "%)");//30 80
        group.children[i].material.color = updateColor;
        if (!wait) {
            wait = true;
        } else {
    ...
    if (frequencyData[i] > 190 && frequencyData[i] < 240) { //bass
        let updateColor = new THREE.Color("rgb(" + 22 + "%, " + 22 + "%, " + 22 + "%)");//30 80
        group.children[i].material.color = updateColor;
    }
    ...
    if (frequencyData[i] > 400) {
        let updateColor = new THREE.Color("rgb(" + 30 + "%, " + 30 + "%, " + 60 + "%)");
        group.children[i].material.color = updateColor;
    }
}
...
function mapAudioInformation(group, shadowGroup) {
    ...
    for (let i = 0; i < planeVertices; i++) {
        if (frequencyData[i] > 0) {
            group.children[i].scale.y = 2 * (frequencyData[i] / 100); //transform group and shadow group objects according to the current frequence
            group.children[i].scale.z = frequencyData[i] / 80;
            group.children[i].scale.x = frequencyData[i] / 70;
            shadowGroup.children[i].scale.y = 2 * (frequencyData[i] / 100);
    ...
    for (var i = 0; i < group.children.length; i++) {
        ...
        curveLine.curve.points[0].x = (frequencyData[i - 1] / 70); //update line that will later be extruded to a new geometry
        ...
        if (frequencyData[0] > 10 && group.children[i]) {
            group.children[i].geometry = new THREE.TubeBufferGeometry(extrudePath, 12, 2, 20); //extrude new geometry
        }
...
function initializeAudio() {
    ...
    document.getElementById("playbutton").addEventListener('click', function () { //play song if user interaction
        audio.play();
        startedAudio = true;
...
function drawFrame(ts) {
    ...
    if (startedAudio) {
        animateCamera();
        wakov(); //calculates random parameter for the camera moevement
        toggleSong === false ? mapAudioInformation(group, shadowGroup) : mapAudioInformation(groupSec, shadowGroupSec); //choose which mesh will be animated
    }
}
